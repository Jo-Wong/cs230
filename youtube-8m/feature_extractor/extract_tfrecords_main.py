...
def pca(res):
    ...
    k=1
    mean = numpy.mean(res, axis=1)
    variance = numpy.var(res, axis=1)
    amax = numpy.amax(res, axis=1)
    stacked = numpy.vstack((mean, variance, amax))
    stacked_centered = (stacked - numpy.mean(stacked, axis=0) ) / numpy.std(stacked, axis=0)
    stacked_centered = stacked_centered.T
    covariance_matrix = numpy.cov(stacked_centered.T)
    ...
    transformed_data = numpy.matmul(stacked_centered, feature_vector)
    l2_norm = numpy.sqrt(numpy.sum(numpy.power(transformed_data,2)))
    transformed_data /= l2_norm
    return transformed_data

def main(unused_argv):
  extractor = feature_extractor.YouTube8MFeatureExtractor(FLAGS.model_dir)
  writer = tf.compat.v1.python_io.TFRecordWriter(FLAGS.output_tfrecords_file)
  total_written = 0
  total_error = 0
  for video_file, labels in csv.reader(open(FLAGS.input_videos_csv)):
    ...
    for rgb in frame_iterator(
      ...
      rgb_features_float.append(features)
    ...
    pklName = video_file.replace('video', 'video_feature').replace('.mp4', '.pkl')
    f = open(pklName, 'wb')
    pickle.dump(pca(rgb_features_float).squeeze(), f, protocol=2)
    f.close()
