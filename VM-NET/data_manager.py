...
class DataManager:
   def __init__(self, dataset_path):
      ...
      self.test_set_x  = DataSet(os.path.join(dataset_path, 'audio_feature/test/combined.pkl'))
      self.test_set_y  = DataSet(os.path.join(dataset_path, 'video_feature/test/combined.pkl'))

      assert self.train_set_x.get_num_set() == self.train_set_y.get_num_set()
      assert self.test_set_x.get_num_set() == self.test_set_y.get_num_set()

      print('Train: %d' % (self.train_set_x.get_num_set()))
      print('Test:  %d' % (self.test_set_x.get_num_set()))
   ...
   def batch_iterator(self, num_batch, is_train):
      if is_train:
         ...
         x = self.test_set_x.list
         y = self.test_set_y.list
         x_data = self.test_set_x.data
         y_data = self.test_set_y.data
         select = self.get_random_n_selection(num_batch, x)
      ...
      while True:
         ...
         for key in keys:
            batch_name.append(x[key])
            assert(x[key] == y[key])
            batch_x.append(x_data[x[key]])
            batch_y.append(y_data[y[key]])

         for key, name in enumerate(batch_name):
            idx = np.where([name.split('_')[0] in t for t in batch_name])[0]
            batch_aff_xy[key, idx] = 1

         yield np.array(batch_x), np.array(batch_y), np.array(batch_aff_xy), np.array(batch_name)
   ...
   def deploy(self):
      ...
      x += self.train_set_x.list
      x_data += list(self.train_set_x.data.values())
      x += self.test_set_x.list
      x_data += list(self.test_set_x.data.values())

      return np.array(x_data), np.array(x)

   def get_batch(self, b, batch_list):
      x, y, aff_xy, name = b[0].next()
      batch_list[0] = x
      batch_list[1] = y
      batch_list[2] = aff_xy
      batch_list[3] = name
